...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s lch-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  lch-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded lch transaction") + "\n" +
              "  lch-tx [options] -create [commands]   " + _("Create hex-encoded lch transaction") + "\n" +
              "\n";
